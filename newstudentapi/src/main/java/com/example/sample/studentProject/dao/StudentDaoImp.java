...
public class StudentDaoImp implements StudentDao {
	...
	public List<Student> getAllStudents() {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createSQLQuery("select * from Students where isActive=true ");
		return query.list();
	}
	...
	public Student findByRollNo(int rollNo) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("FROM Student WHERE id = :rollNo and isActive=true");
